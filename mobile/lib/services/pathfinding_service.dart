...
class PathfindingService {
  ...
  // ── Find route away from danger (DEFAULT-SAFE MODEL) ─────────────────────

  /// Routes the user away from the current danger zone.
  /// In the default-safe model:
  /// - A "safe" node = any node whose zoneType is NOT restricted/caution
  /// - If the user is already safe (the default), returns immediately
  /// - If ALL reachable nodes are in danger zones → returns SOS fallback
  NavigationResult findRouteToSafety({
    ...
    // Already safe (the default state) — no routing needed
    if (!start.zoneType.isDanger) {
      return NavigationResult(
        ...
        estimatedMinutes: 0,
        pathFound: true,
        message: 'You are in a safe area. No action needed.',
      );
    }

    // Find nodes NOT in danger zones (safe by default)
    final safeNodes = _nodes.values
        .where((n) => !n.zoneType.isDanger)
        .toList();

    // NULL-ROUTE FALLBACK: All reachable nodes are in danger zones
    if (safeNodes.isEmpty) {
      return const NavigationResult(
        ...
        estimatedMinutes: 0,
        pathFound: false,
        message: 'All nearby areas are in danger zones. '
            'Activate SOS immediately and stay in place until rescue arrives.',
      );
    }
    ...
    // A* couldn't find a connected path — SOS fallback
    if (rawPath == null) {
      return const NavigationResult(
        path: [],
        offlineGeometries: [],
        ...
        estimatedMinutes: 0,
        pathFound: false,
        message: 'No connected safe route found. '
            'Activate SOS and retrace your steps carefully.',
      );
    }
