...
class LocationProvider with ChangeNotifier {
  ...
  final GeofencingEngine _geofencing = locator<GeofencingEngine>();
  final BreadcrumbManager _breadcrumbs = locator<BreadcrumbManager>();
  ...
  void _startSafetyMonitoring() {
    _safetyTimer?.cancel();
    _safetyTimer = Timer.periodic(const Duration(seconds: 60), (timer) async {
  ...
  void _processPing(Position rawPos) {
    ...
    // 1. Accuracy Filter — silently drop noisy readings without rebuilding UI
    if (rawPos.accuracy > 20.0 && rawPos.accuracy != 1.0) {
      return; // PERF FIX: removed notifyListeners() — nothing changed
    }
  ...
  Future<void> stopTracking() async {
    ...
    _safetyTimer?.cancel();
    // locator<FallDetectionService>().stopMonitoring();
    _isTracking = false;
    _isLocationActive = false;
    notifyListeners();
  }
