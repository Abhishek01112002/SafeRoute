...
void main() {
  group('MeshPacket Tests', () {
    test('MeshPacket serialization and deserialization', () {
      ...
      expect(map['sourceId'], packet.sourceId.hashCode.toString()); // map stores hash string
      expect(map['type'], MeshPacketType.SOS_ALERT.index);
      
      final deserialized = MeshPacket.fromMap(map);
      
      expect(deserialized.packetId, packet.packetId);
    ...
    test('MeshPacket copyWith hopCount logic', () {
      ...
      expect(relayedPacket.packetId, packet.packetId); // MUST be same ID so caching prevents loops
  ...
  group('MeshNode Tests', () {
    test('MeshNode creation from scan', () {
      ...
      expect(node.rssi, -65);
      expect(DateTime.now().difference(node.lastSeen).inSeconds, 0); // Roughly now
    });
  });
}
