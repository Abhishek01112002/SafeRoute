...
class TestSosTriggerHappyPath:
    ...
    def test_sos_trigger_accepts_unknown_location(self, client, tourist_auth_header):
        payload = {
            "latitude": None,
            "longitude": None,
            ...
            "idempotency_key": "unknown-location-sos-1",
        }
        resp = client.post("/sos/trigger", json=payload, headers=tourist_auth_header)

        assert resp.status_code == 202, (
            f"Expected 202 for locationless SOS, got {resp.status_code}: {resp.text}"
        )
        body = resp.json()
        assert body["status"] == "queued"
        assert body["delivery_state"] == "PENDING"
