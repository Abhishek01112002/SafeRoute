...
class _SosScreenState extends State<SosScreen>
  ...
  void _triggerSos() async {
    ...
    // ISSUE #4 FIX: Add 10-second timeout for location acquisition with fallback
    Position? location;
    try {
      ...
      // Timeout or error - fallback to last known position from LocationProvider
      location = locProv.currentPosition;
      if (location == null && mounted) {
        setState(() => _sosStatus = "NO POSITION AVAILABLE — SOS CANCELLED");
        return;
      }
      if (mounted) setState(() => _sosStatus = "USING LAST KNOWN POSITION...");
    }
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Text("PROCEED WITH SOS?",
            style: TextStyle(
                color: theme.colorScheme.onSurface,
                fontWeight: FontWeight.w900,
                fontSize: 16)),
        content: Text(
            "Emergency services and contacts will receive your live coordinates.",
            style: TextStyle(
        ...
        actions: [
          ...
          TextButton(
              ...
              child: const Text("SEND SOS",
                  style: TextStyle(
                      color: AppColors.zoneRed, fontWeight: FontWeight.w900))),
        ],
      ),
    );
      ...
      try {
        ...
        if (isOnline) {
          try {
            ...
            await apiService
                .sendSosAlert(location.latitude, location.longitude, "MANUAL",
                    touristId: tourist.touristId)
                .timeout(const Duration(seconds: 10));
            if (mounted)
              setState(() {
                _isOfflineStored = false;
                _sosStatus = "SIGNAL CONFIRMED ✓";
              });
          } catch (e) {
            if (mounted)
              setState(
                  () => _sosStatus = "NETWORK UNSTABLE — BUFFERING LOCALLY...");
            await dbService.saveSosEvent(
                touristId: tourist.touristId,
            ...
            if (mounted)
              setState(() {
                _isOfflineStored = true;
                _sosStatus = "SOS BUFFERED OFFLINE";
              });
          }
        } else {
          await dbService.saveSosEvent(
          ...
          if (mounted)
            setState(() {
              _isOfflineStored = true;
              _sosStatus = "QUEUED — MESH RELAY ACTIVE";
            });
        }
      } catch (e) {
        if (mounted)
          setState(() => _sosStatus = "CRITICAL ERROR — SOS BUFFERED");
      }
    } else {
      if (mounted)
        setState(() {
        ...
        children: [
          ...
          ListView(
            padding: const EdgeInsets.symmetric(
                horizontal: AppSpacing.l, vertical: AppSpacing.xxl),
            children: [
  ...
  Widget _buildSafetyStatus(dynamic tourist) {
    Color riskColor =
        (tourist.riskLevel == "HIGH" || tourist.riskLevel == "CRITICAL")
            ? AppColors.zoneRed
            : (tourist.riskLevel == "MODERATE"
      ...
      child: Row(
        children: [
          ...
          Column(
            ...
            children: [
              Text("${tourist.riskLevel} ALERT STATUS",
                  style: TextStyle(
                      fontWeight: FontWeight.w900,
                      color: riskColor,
                      fontSize: 10)),
              Text(
                  tourist.offlineModeRequired
                      ? "HYBRID DEFENSE ACTIVE"
                      : "STANDARD MONITORING",
                  style: TextStyle(
                      fontSize: 8, color: riskColor.withOpacity(0.5))),
            ],
          ),
        ],
      ),
        ...
        children: [
          ...
          if (location != null)
            Row(
              ...
              children: [
                Text("${location.latitude.toStringAsFixed(6)}° N",
                    style: TextStyle(
                        color: theme.colorScheme.onSurface,
                        fontWeight: FontWeight.w900)),
                Text("${location.longitude.toStringAsFixed(6)}° E",
                    style: TextStyle(
                        color: theme.colorScheme.onSurface,
                        fontWeight: FontWeight.w900)),
              ],
          ...
          children: [
            Container(
              ...
              decoration: BoxDecoration(shape: BoxShape.circle, boxShadow: [
                BoxShadow(
                    color: (isSosActive ? AppColors.zoneRed : AppColors.primary)
                        .withOpacity(0.1),
                    blurRadius: 60)
              ]),
            ),
            SizedBox(
                ...
                child: CircularProgressIndicator(
                    ...
                    valueColor: AlwaysStoppedAnimation(
                        isSosActive ? AppColors.zoneRed : AppColors.primary))),
            AnimatedBuilder(
              animation: _pulseController,
              builder: (ctx, child) => Transform.scale(
                  scale:
                      isSosActive ? 1.0 + (_pulseController.value * 0.05) : 1.0,
                  child: child),
              child: Container(
                width: 170,
                height: 170,
                ...
                child: Column(
                    ...
                    children: [
                      ...
                      Text(isSosActive ? "ACTIVE" : "HOLD 3s",
  ...
  Widget _statusBadge(String label, bool active) {
    return Container(
      ...
      decoration: BoxDecoration(
          ...
          color: active
              ? AppColors.zoneGreen.withOpacity(0.1)
              : Colors.transparent,
          border:
              Border.all(color: active ? AppColors.zoneGreen : Colors.white10)),
      child: Text(label,
          style: TextStyle(
              fontSize: 8,
              color: active ? AppColors.zoneGreen : Colors.white38)),
    );
  }
  ...
  Widget _buildDirectory(dynamic tourist, ThemeData theme) {
    return Column(
      ...
      children: [
        ...
        _contactItem(tourist.emergencyContactName.toUpperCase(),
            tourist.emergencyContactPhone, false, theme),
      ],
    );
  }

  Widget _contactItem(
      String name, String phone, bool official, ThemeData theme) {
    return EliteSurface(
      margin: const EdgeInsets.only(bottom: AppSpacing.s),
      child: Row(
        children: [
          Icon(official ? Icons.emergency_rounded : Icons.person_rounded,
              color: theme.colorScheme.primary, size: 18),
          const SizedBox(width: AppSpacing.m),
          Expanded(
              child: Text(name,
          ...
          IconButton(
              ...
              icon: const Icon(Icons.call_rounded,
                  color: AppColors.zoneGreen, size: 18)),
        ],
      ),
    );
  }
...
class _SosRadarBgState extends State<_SosRadarBg>
    with SingleTickerProviderStateMixin {
...
class _RadarPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawCircle(Offset(size.width / 2, size.height / 2.5),
        progress * size.width * 0.8, paint);
    canvas.drawCircle(Offset(size.width / 2, size.height / 2.5),
        ((progress + 0.5) % 1.0) * size.width * 0.8, paint);
  }
