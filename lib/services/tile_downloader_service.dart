...
class TileDownloaderService {
  static final TileDownloaderService _instance = TileDownloaderService._internal();
  ...
  math.Point<int> _latLonToTile(LatLng pos, int zoom) {
    ...
    final y = ((1 - (math.log(math.tan(pos.latitude * math.pi / 180) + 1 / math.cos(pos.latitude * math.pi / 180)) / math.pi)) / 2 * n).toInt();
    return math.Point(x, y);
  }

  /// Downloads all tiles for a region between specified zoom levels.
  Stream<DownloadProgress> downloadRegion(String regionKey, {int minZoom = 12, int maxZoom = 16, required String urlTemplate}) async* {
    ...
    for (final tileKey in tileQueue) {
      ...
      if (existing != null) {
        downloaded++;
        if (downloaded % 10 == 0) {
          yield DownloadProgress(regionName: regionKey, total: total, downloaded: downloaded);
        }
        continue;
      }
    ...
    yield DownloadProgress(regionName: regionKey, total: total, downloaded: total, isComplete: true);
  }
}
