...
*   **Multi-Channel Alerts:** When a critical SOS is fired, the backend dispatches Firebase Cloud Messaging (FCM) push notifications, alongside SMS fallbacks (via Fast2SMS/Twilio) to the relevant authorities.
...
cp .env.example .env 
# Add your JWT_SECRET and API keys to .env
...
## 🔒 Security & Privacy
SafeRoute employs localized data caching and anonymized mesh-node identification to prioritize user privacy. Location data is only dispatched to the centralized server during explicit SOS events or while navigating active `RESTRICTED` zones.
